  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(`${apiBaseUrl}/movies/viewmovie?id=${movieID}`, {
          method: 'GET',
          headers: {
  ...
  return (
    <div className='ml-10 mr-5'>
      ...
      <div className='flex flex-row gap-3 mt-5 items-center'>
        ...
        <h3>|</h3>
        <h4>Duration: <span className='text-[#e6ad35] font-bold'>{movie.durationHours} hrs {movie.durationMinutes} min</span></h4>
        <h3>|</h3>
        <h4 className='font-bold'>{movie.genres.map(genre => genre.name).join(", ")}</h4>
      </div>
